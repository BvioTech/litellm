...
def _stale_rewrite_paths(
    current: MCPArgumentValue,
    scanned_texts: tuple[tuple[MCPArgumentPath, str], ...],
    replacements: Mapping[MCPArgumentPath, str],
) -> tuple[MCPArgumentPath, ...]:
    """Paths this guardrail wants to rewrite that another guardrail already rewrote.
    ...
    of them rewriting one leaf cannot be merged: writing either result discards the
    other guardrail's redaction. Detect that by comparing the leaf as it stands now
    against the text this guardrail was given.
    """
    scanned = dict(scanned_texts)
    current_texts = dict(_collect_argument_texts(current))
    return tuple(path for path in replacements if current_texts.get(path, scanned[path]) != scanned[path])


def _conflicting_rewrite(paths: tuple[MCPArgumentPath, ...]) -> HTTPException:
    return HTTPException(
        status_code=400,
        detail={
            "error": (
                "Content blocked: two guardrails running concurrently rewrote the same MCP tool call "
...
class MCPGuardrailTranslationHandler(BaseTranslation):
    ...
    async def process_input_messages(
        ...
        current_arguments = data.get("mcp_arguments") or data.get("arguments")
        if not isinstance(current_arguments, dict):
            current_arguments = mcp_arguments
        stale = _stale_rewrite_paths(current_arguments, argument_texts, replacements)
        if stale:
            raise _conflicting_rewrite(stale)
        masked_arguments = _replace_argument_texts(current_arguments, replacements)
