        ...
        for param, value in non_default_params.items():
            ...
            if param == "thinking" and "openai.gpt-5" not in model:
                if (
                    ...
                    # An application inference profile ARN carries no model id, so
                    # the probe above always says "not adaptive" and would downgrade
                    # an explicit adaptive request to a fixed 2048-token budget while
                    # `output_config.effort` is still forwarded — a contradictory
                    # pair. Forward as-is and let Bedrock validate.
