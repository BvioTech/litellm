...
async def test_legacy_local_tool_fallback_fails_closed_when_prefix_names_no_server(
    ...
    """Defensive arm, not a live case: a non-empty prefix that passed the
    server-level check always matches one of `allowed_mcp_servers` by name, so
    this is unreachable today. The test forces the mismatch (patching the
    server-level check, which would otherwise refuse first) to pin the
    fail-closed behavior, so a later refactor cannot make the branch dispatch
    with no server to evaluate a tool ceiling against.
    """
    from fastapi import HTTPException
